...
def get_temperature(iteration, epoch, total_batches, temp_epoch=20, temp_init_value=1.0):
    """
    基于余弦退火策略的温度系数生成器

    参数:
        iteration (int): 当前epoch内的迭代步数(从1开始)
        epoch (int): 当前epoch数(从0开始)
        total_batches (int): 每个epoch的总批次数
        temp_epoch (int): 温度调整周期，默认20个epoch后稳定
        temp_init_value (float): 初始温度值，默认1.0
    ...
    # 当前进度比例 (考虑跨epoch的连续进度)
    progress = min(epoch + iteration / total_batches, temp_epoch) / temp_epoch

    # 余弦退火计算
    current_temp = temp_init_value * (math.cos(math.pi * progress) + 1) / 2

    # 最低温度保护
    return max(current_temp, 0.1) if temp_init_value > 0.1 else current_temp
    ...
    def _setup_train(self, world_size):
        ...
        if self.args.kd_loss_type == 'feature' or self.args.kd_loss_type == 'all':
            ...
            with torch.no_grad():
                ...
                _ = self.model(inputs)
            self.kd_feature_loss = FeatureLoss([i.size(1) for i in s_feature], [i.size(1) for i in t_feature],

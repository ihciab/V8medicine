...
def bbox_iou(box1, box2, xywh=True,
    ...
    # iou = inter / union # ori iou
    # (x1, y1, w1, h1), (x2, y2, w2, h2) = box1.chunk(4, -1), box2.chunk(4, -1)
    # w1_, h1_, w2_, h2_ = w1 / scale, h1 / scale, w2 / scale, h2 / scale
    # diag1 = torch.sqrt(w1_ ** 2 + h1_ ** 2)  # 对角线长度计算
    # diag2 = torch.sqrt(w2_ ** 2 + h2_ ** 2)
    # avg_diag = (diag1 + diag2) / 2
    # use_ciou = CIoU or (avg_diag.mean() >= size_threshold)

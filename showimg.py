...
def prune(args):
    ...
    for i in range(args.iterative_steps):
        ...
        #distillmodel.model.load_state_dict(model.model.state_dict())  # 关键步骤：显式加载剪枝权重



        distillmodel.save_model = save_model_v2.__get__(distillmodel)
        distillmodel.final_eval = final_eval_v2.__get__(distillmodel)

        ####
        # 问题出在setup prune函数中 要加载预训练的权重
        print("distill Begin")
        distillmodel.distill(weights=model.model)
        if hasattr(distillmodel, 'criterion'):
            distillmodel.criterion = distillmodel.criterion.to('cuda:0')

            # post fine-tuning validation

        if RANK in (-1, 0):  # RANK为-1（单卡）或0（分布式主卡）
            # 加载训练过程中保存的最佳模型
            print("distill End Save Config")
            model.model, _ = attempt_load_one_weight(str(distillmodel.best))
            model.overrides = model.args  # 更新配置参数为最佳模型的参数
            model.metrics = getattr(distillmodel.validator, 'metrics', None)  # 记录验证指标
        ...
        # post fine-tuning validation
        #model.trainer.best = distilled_model_path
        pruning_cfg['name'] = f"step_{i}_post_val"
        pruning_cfg['batch'] = 1
...
if __name__ == "__main__":
    ...
    parser.add_argument('--iterative-steps', default=2, type=int, help='Total pruning iteration step')
